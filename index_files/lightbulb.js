...
config.IOT_THING_NAME           = "lightbulb_jp";
...
{
    $("#infoLabel").text("Retrieving Cognito Id");
    ...
    AWS.config.credentials.get(function(err)
    {
        if (!err)
        {
            $("#infoLabel").text("Connecting to AWS IoT");
...
function setLocalBulbStateForPercentage(percentage)
{
    ...
    $("#infoLabel").text(percentage + "% brightness");
...
function WebSocketService()
{
    ...
    function conn(iotEndpoint, iotRegion, thingName, callbackConnect, callbackUpdate) {
        ...
        if (service.mqttClient == null) {
            var requestUrl = prepareWebsocketUrl({ host: service.iotEndpoint, region: service.iotRegion, debug: true }, AWS.config.credentials.accessKeyId, AWS.config.credentials.secretAccessKey, AWS.config.credentials.sessionToken);
    ...
    function checkUpdate(topic, message)
    {
        ...
        if (service.callbackUpdate != null && topic.indexOf('/shadow/update/accepted') != -1)
        {
            //
            // Attempt to get the desired brightness
            //
